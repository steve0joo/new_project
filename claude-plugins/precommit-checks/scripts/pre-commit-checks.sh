#!/usr/bin/env bash
# Pre-commit checks, shipped as a Claude Code plugin PreToolUse(Bash) hook.
#
...
# else passes straight through. On a commit it runs the project's checks and, if any
...
#
# The checks run against the user's project (CLAUDE_PROJECT_DIR), not the plugin dir.
# Exit codes: 0 = allow the commit, 2 = block it.

set -uo pipefail

# Run in the project where Claude Code was launched (where the commit happens).
